		...
		if size > 0 then
			for i = 0, size - 1 do
				...
				if eventAtIndex == GetHashKey("EVENT_ENTITY_DAMAGED") then
					...
					local eventDataStruct = DataView.ArrayBuffer(72) -- buffer must be 8*eventDataSize or bigger
					...
					if is_data_exists then
                        PlayerDamaged(eventDataStruct:GetInt32(16) --[[ weapon hash ]], eventDataStruct:GetInt32(32) --[[ damage amount ]])
					end
				end
			end
		end
...
PlayerDamaged = function(weaponHash, damageAmount)
    for k,v in pairs(weapons) do
        if weaponHash == v[1] then
            local stresschance = math.random(1,4)
            if stresschance == 3 then
                TriggerServerEvent("redemrp_status:server:AddStress", math.random(3,6))
            end
        end
    end
end
...
local EagleEye = false
local EagleEyeTimer = nil
local EagleEyeCooldown = nil
function EagleEyeTonic()
    Drink()
    EagleEyeTimer = GetGameTimer()
    EagleEye = true
    Citizen.InvokeNative(0xA63FCAD3A6FEC6D2, PlayerId(), true) -- EnableEagleEye
    Wait(10)
    Citizen.InvokeNative(0x2C41D93F550D5E37, PlayerId(), false) -- EagleEyeSetColor
    RedEM.Functions.NotifyRight( "Eagle eye enabled for 3 minutes!", 5000)
    Citizen.CreateThread(function()
        while true do
            Wait(5000)
            if GetGameTimer() - EagleEyeTimer > 180000 then
                RedEM.Functions.NotifyRight( "Your eagle eye ran out!", 5000)
                Citizen.InvokeNative(0xA63FCAD3A6FEC6D2, PlayerId(), false) -- EnableEagleEye
                Citizen.InvokeNative(0x2C41D93F550D5E37, PlayerId(), false) -- EagleEyeSetColor
                break
            end
        end
...
function EagleEyeTonicChew()
    EagleEyeTimer = GetGameTimer()
    EagleEye = true
    Citizen.InvokeNative(0xA63FCAD3A6FEC6D2, PlayerId(), true) -- EnableEagleEye
    Wait(10)
    Citizen.InvokeNative(0x2C41D93F550D5E37, PlayerId(), false) -- EagleEyeSetColor
    RedEM.Functions.NotifyRight( "Eagle eye enabled for 2 minutes!", 5000)
    Citizen.CreateThread(function()
        while true do
            Wait(5000)
            if GetGameTimer() - EagleEyeTimer > 120000 then
                RedEM.Functions.NotifyRight( "Your eagle eye ran out!", 5000)
                Citizen.InvokeNative(0xA63FCAD3A6FEC6D2, PlayerId(), false) -- EnableEagleEye
                Citizen.InvokeNative(0x2C41D93F550D5E37, PlayerId(), false) -- EagleEyeSetColor
                break
...
function GEagleEyeTonic()
    Drink()
    EagleEyeTimer = GetGameTimer()
    EagleEye = true
    Citizen.InvokeNative(0xA63FCAD3A6FEC6D2, PlayerId(), true) -- EnableEagleEye
    Wait(10)
    Citizen.InvokeNative(0x2C41D93F550D5E37, PlayerId(), true) -- EagleEyeSetColor
    RedEM.Functions.NotifyRight( "Green eagle eye enabled for 3 minutes!", 5000)
    Citizen.CreateThread(function()
        while true do
            Wait(5000)
            if GetGameTimer() - EagleEyeTimer > 180000 then
...
function Cigar()
    ...
    TaskItemInteraction_2(PlayerPedId(), -1199896558, propEntity, GetHashKey('P_CIGAR01X_PH_R_HAND'), GetHashKey('QUICK_SMOKE_CIGAR_LEFT_HAND'), 1, 0, -1.0)
end

function Cigarette()
    TriggerEvent("redemrp_inventory:closeinv")
    local propEntity = CreateObject(GetHashKey('p_cigar01x'), GetEntityCoords(PlayerPedId()), false, true, false, false, true)
    TaskItemInteraction_2(PlayerPedId(), -1199896558, propEntity, GetHashKey('P_CIGAR01X_PH_R_HAND'), GetHashKey('QUICK_SMOKE_CIGARETTE_LEFT_HAND'), 1, 0, -1.0)
end]]
...
RegisterNetEvent("redemrp_status:client:PipeMenu", function(hasOpium, hasWeed, hasTobacco, hasBTobacco, hasIndTobacco)
    TriggerEvent('redemrp_inventory:closeinv')

    local elements = {}
    if hasOpium then
        table.insert(elements, {label = "Smoke <strong>Dried Opium</strong>", value = "Opium", desc = "Smoke Dried Opium"})
    end
    if hasWeed then
        table.insert(elements, {label = "Smoke <strong>Weed</strong>", value = "Weed", desc = "Smoke Weed"})
    end
    if hasTobacco then
        table.insert(elements, {label = "Smoke <strong>Tobacco</strong>", value = "Tobacco", desc = "Smoke Tobacco"})
    end
    if hasBTobacco then
        table.insert(elements, {label = "Smoke <strong>Burnley Tobacco</strong>", value = "BTobacco", desc = "Smoke Burnley Tobacco"})
    end
    if hasIndTobacco then
        table.insert(elements, {label = "Smoke <strong>Native Tobacco</strong>", value = "IndTobacco", desc = "Smoke Native Tobacco"})
    end

    if #elements == 0 then
        return RedEM.Functions.NotifyRight("You have nothing to smoke!", 5000)
    end

    MenuData.Open('default', GetCurrentResourceName(), 'pipemenu_smoking',{
        title    = "Pipe Menu",
        subtext    = "What do you want to smoke?",
        align    = 'top-right',
        elements = elements,
    },
    function(data, menu)
        TriggerServerEvent("redemrp_status:server:Smoke"..data.current.value)
        menu.close()
    end,
    function(data, menu)
        menu.close()
