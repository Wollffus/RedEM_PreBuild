...
namespace SaltyClient
{
    internal class VoiceManager : BaseScript
    {
        ...
        private async Task OnStateUpdateTick()
        {
            if (this.IsConnected && this.IsIngame)
            {
                ...
                foreach (VoiceClient client in this.VoiceClients)
                {
                    ...
                    if (API.GetRoomKeyFromEntity(ped.Handle) != API.GetRoomKeyFromEntity(playerPed.Handle) && !API.HasEntityClearLosToEntity(playerPed.Handle, ped.Handle, 17))
					{
						muffleIntensity = 7;
					}
                    playerStates.Add(
        ...
        public void ToggleVoiceRange()
        {
            ...
            BaseScript.TriggerEvent("redemrp_notification:start", $"New voice range is {this.VoiceRange} metres.", 5, "info");
        }
        #endregion
